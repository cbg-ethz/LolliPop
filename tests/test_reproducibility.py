    ...
    try:
        ...
        for i in range(1, n_runs):
            ...
            try:
                ...
                print(
                    f"Multi-location seeding difference found between run 0 and run {i}:"
                )
                ...
                if not comparison.empty:
                    ...
                    for idx in differing_indices[:10]:  # Show first 10 differences
                        ...
                        for col in row_comparison.index.levels[0]:  # Get column names
                            if pd.notna(row_comparison[col, "self"]) or pd.notna(
                                row_comparison[col, "other"]
                            ):
                                val_first = (
                                    row_comparison[col, "self"]
                                    if pd.notna(row_comparison[col, "self"])
                                    else df_first.loc[idx, col]
                                )
                                val_next = (
                                    row_comparison[col, "other"]
                                    if pd.notna(row_comparison[col, "other"])
                                    else df_next.loc[idx, col]
                                )
                                print(f"  {col}:")
                ...
                raise AssertionError(
                    f"Multi-location seeding reproducibility failed: {e}"
                )
